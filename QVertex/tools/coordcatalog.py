    ...
    def prepare_data(self):
        ...
        if len(self.features) > 1:
            ...
            for feat in self.features:
                ...
                self.area.append(math.ceil(gt.area(), 0))
                self.perimeter.append(math.ceil(gt.length(), 2))
                #self.zu = []
            ...
            gt = QgsGeometry(geom)
            gt.transform(self.transform)
            self.area.append(math.ceil(gt.area()))
            self.perimeter.append(math.ceil(gt.length()))
            self.parse_polygon(geom.asMultiPolygon()[0])
